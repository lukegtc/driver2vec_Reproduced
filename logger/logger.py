...
TEMP_FOLDER = os.path.join(GC_HOME,'temp_store')
...
class Logger(object):
    ...
    def log_numpy(self, output, filename):
        with Path(self.file_outputs_dir / (filename + '.npy')).open('wb') as f:
            np.save(f, output)

    def log_data(self, data, filename):
        # filehandle = (self.file_outputs_dir / (filename + '.pickle'))\
        #                                                       .open('wb')
        with Path(self.file_outputs_dir / (filename + '.pickle')).open('wb') as f:
            pickle.dump(data, f)

    def log_image(self, fig, filename):
        # filehandle = GCOpen(self.file_outputs_dir / filename, 'wb')
        with Path(self.file_outputs_dir / filename).open('wb') as f:
            fig.savefig(f)
